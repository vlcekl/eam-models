...
siref = 3.12217297613
epref = 0.19639051593331739961 * 4.184
qref = 1.082914
...
def f_his(p, sa, sc, sq, hhh, hrf, hee, pi, pa, pc, pq, s):
    ...
    s2g = (dh[0]**2 + 4.0*dh[1]**2 + 4.0*dh[2]**2)/9.0
...
for i in range(nsig):
    so = smin + ds*float(i)
    for j in range(neps):
    	eo = emin + de*float(j) # in kJ/mol
        par_in = [so, eo, qo]
	dloss = f_his(par_in,sa,sc,sq,hhh,hrf,hee,pi,pa,pc,pq,scl)
        print so, eo, dloss
